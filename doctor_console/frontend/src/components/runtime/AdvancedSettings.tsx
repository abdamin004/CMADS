...
export function AdvancedSettings({ value, onChange, precisionRows, precisionN }: Props) {
  ...
  return (
    <div className="runtime-solo__advanced-body">
      {/* Accuracy mode is fixed at "Multi-level memory" (full 7-agent pipeline) —
          the single-LLM baseline path is not user-selectable. The field stays in
          the type so callers (RuntimeHero, PatientBuilderEditor) still pass it
          through to the backend, but the UI does not surface it. */}
