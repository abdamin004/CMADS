...
\begin{tikzpicture}[
  ...
  % Row of 6 stages below (left to right)
...
The six stages are summarised below.
\begin{itemize}
...
All inter-agent communication is mediated through a typed shared memory
implemented as a \texttt{TypedDict} named \texttt{PipelineState}. Each field
of \texttt{PipelineState} is a separate namespace with an explicit reducer
function that defines how partial updates from different agents are merged.
This design was chosen over a naive key--value store because it makes
concurrent-write conflicts impossible by construction: if two agents write to
the same namespace in parallel, the reducer deterministically resolves the
merge rather than producing an undefined last-writer-wins state.

Five namespaces partition the shared memory:
\begin{itemize}
  ...
  using a write-own / read-upstream access pattern.
  \item \texttt{conflicts} --- an append-only log of contradictions detected
  between agents, populated by the orchestrator's post-stage diff engine.
  \item \texttt{scratchpad} --- per-agent ephemeral notes, private to the
  writing agent and not visible to downstream agents.
  \item \texttt{execution\_trace} --- an append-only invocation log that
  records the status, duration, and error of every agent run.
\end{itemize}

Table~\ref{tab:memory_access} summarises the read/write access pattern of
each agent across the five namespaces. The key invariant is that every
agent writes only to its own slot in \texttt{agent\_outputs} and reads from
the slots of upstream agents, which eliminates concurrent-write conflicts by
construction.

\begin{table}[ht]
\centering
\caption{Shared memory access pattern. R = reads, W = writes to own slot.}
\label{tab:memory_access}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Agent} & \texttt{patient\_context} & \texttt{agent\_outputs} & \texttt{conflicts} & \texttt{scratchpad} & \texttt{execution\_trace} \\
\midrule
EHR Analyst          & R & W      & R & RW & W \\
Lab Interpreter      & R & W      & R & RW & W \\
Diagnostic Reasoning & R & R~/~W  & R & RW & W \\
Clinical Reviewer    & R & R~/~W  & R & RW & W \\
Diagnostic Refiner   & R & R~/~W  & R & RW & W \\
LLM Evaluator        & R & R~/~W  & -- & RW & W \\
Treatment Planning   & R & R~/~W  & -- & RW & W \\
\bottomrule
\end{tabular}%
}
\end{table}
