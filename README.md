...
└── data/                   # Patient data (gitignored, regenerated)
    └── gold/batches/       # Cohort batch definitions (tracked)
```
...
### Run the MAS pipeline on a single patient
```bash
python -c "from src.orchestrator.graph import run_single_patient; run_single_patient('<patient-uuid>')"
```
...
Data is regenerated by running the pipeline scripts. See `pipeline/` for implementation.
