...
| **4. Case-Based** | cross-session · vector recall | Qdrant collection `patient_cases` *(new)* | Each finished patient (demographics + conditions + meds + labs) is embedded with BioLORD; the Diagnostic agent recalls the top-K most similar past patients as a Bayesian-style prior |

*NICE clinical guidelines still live in the separate `nice_guidelines` Qdrant collection (used by the Treatment agent for drug/contraindication lookup) — they are external reference knowledge, not a memory tier.*
